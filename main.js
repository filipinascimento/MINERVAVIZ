...
export class HeliosMap {
	...
	async setupLinks() {
		...
		edgesIndices.forEach((index) => {
			...
			c.opacity = 0.40 * weight / weightMinMaxGlobal[1] + 0.40 * weight / weightMinMax[1] + 0.2;
			// c.opacity=1.0;
			if (partialOpacity) {
				c.opacity = 0.1;
			}
...
let projectsOrder = ["coproject","coauthor"];
...
if(!window.location.hash){
	window.location.hash = "#COPROJECT";
}else{
	locationHashChanged();
}
